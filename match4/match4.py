...
def cleanData(data, key, img_dict):
    clean_data = []
    index = 0
    for item in data:
        if item[1] != key:
            number = img_dict[item[0]]
            offset = float(item[2])
            clean_data.append({'number': number,
                             'offset': offset,
                             'index': index })
            index += 1
...
def reNumber(clean_num):
    for item in clean_num:
        # //元素的相对浮动是相对原来位置的偏移
        new_index = item['index'] + item['offset']/11.5
        item['index'] = new_index
    clean_num = sorted([item for item in clean_num] , key=lambda x:x['index'])
    newNum = int(''.join([str(item['number']) for item in clean_num]))
    return newNum


sum = 0
for page in range(1, 6):
    response = requests.get(f'https://match.yuanrenxue.cn/api/match/4?page={page}', headers=headers, verify=False)
    data = response.json()
    data_info = response.json().get('info')

    with open("match4.js", "r", encoding="utf-8") as f:
        js_code = f.read()

    ctx     = execjs.compile(js_code)
    j_key   = ctx.call("get_j_key", data)
    tds      = re.findall(r'<td>(.*?)</td>', data_info)

    for imgs in tds:
        number_block_list = re.findall(r'src="(.*?)" class="img_number (.*?)" style="left:(.*?)px', imgs)
        num_info = cleanData(number_block_list, j_key, img_dict)
        sum += reNumber(num_info)
print(sum)
